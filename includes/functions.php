...
function route_url(string $path = '/'): string
{
    ...
    $routePath = '/' . ltrim((string)($parts['path'] ?? '/'), '/');
    $routePath = preg_replace('#/+#', '/', $routePath) ?: '/';

    if ($routePath === '/' || $routePath === '/index.php' || $routePath === '/index') {
        $cleanPath = '/trang-chu';
    } else {
        $cleanPath = preg_replace('#\.php$#i', '', $routePath) ?: $routePath;
    }

    $url = rtrim(BASE_URL, '/') . $cleanPath;
...
function product_detail_path(array $product): string
{
    ...
    if ($categorySlug === '' && !empty($product['category_name'])) {
        $categorySlug = slugify((string)$product['category_name']);
    }
    if ($categorySlug === '') {
    ...
    if ($productSlug === '' && !empty($product['product_name'])) {
        $productSlug = slugify((string)$product['product_name']);
    }
    if ($productSlug === '') {
        $productSlug = 'sp-' . (int)($product['id'] ?? 0);
    }
...
{
    return rtrim(BASE_URL, '/') . product_detail_path($product);
}

function get_product_by_slug(string $categorySlug, string $productSlug, bool $onlyActive = false): ?array {
    $sql = 'SELECT p.*,
